...
class ExpenseService {
    async create(userId, data) {
        ...
        return prisma_js_1.default.expense.create({
            data: {
                amount: data.amount,
    ...
    async findAll(userId, filters) {
        const { startDate, endDate, categoryId, minAmount, maxAmount, search, page = 1, limit = 10, sortBy = "date", sortOrder = "desc", } = filters;
        const where = {
            userId,
    ...
    async findById(userId, id) {
        const expense = await prisma_js_1.default.expense.findFirst({
            where: { id, userId },
            include: { category: true },
        });
        if (!expense) {
    ...
    async update(userId, id, data) {
        ...
        return prisma_js_1.default.expense.update({
            where: { id },
            data,
            include: { category: true },
        });
    ...
    async getStats(userId, startDate, endDate) {
        ...
        const [expenses, categories] = await Promise.all([
            ...
            prisma_js_1.default.category.findMany({ where: { userId } }),
        ]);
